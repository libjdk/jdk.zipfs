	...
	if (var$0 || $nc(options)->contains($StandardOpenOption::APPEND)) {
		...
		ensureOpen();
		{
			...
			try {
				...
				bool var$6 = !options->contains($StandardOpenOption::CREATE);
				if (var$6 && !options->contains($StandardOpenOption::CREATE_NEW)) {
					$throwNew($NoSuchFileException, $(getString(path)));
				}
				checkParents(path);
		...
		beginRead();
		{
			...
			$var($SeekableByteChannel, var$9, nullptr);
			bool return$8 = false;
			try {
				ensureOpen();
				$var($ZipFileSystem$Entry, e, getEntry(path));
				...
				}
				{
					$var($InputStream, is, getInputStream(e));
					{
						...
						$var($SeekableByteChannel, var$12, nullptr);
						bool return$11 = false;
						try {
							try {
								$assign(var$12, $new($ByteArrayChannel, $($nc(is)->readAllBytes()), true));
								return$11 = true;
								goto $finally3;
							} catch ($Throwable&) {
								$var($Throwable, t$, $catch());
